...
class HillCurve(BaseCurve):
    ...
    def build_mu(
        ...
        mu_log_gamma_global = pm.Normal('mu_log_gamma_global', mu=priors.mu_log_gamma_value, sigma=priors.mu_log_gamma_scale)
        ...
        if has_valid_cluster:
            ...
            parent_gamma_cluster = log_gamma_cluster[safe_cluster_idx]
        ...
        gamma_k = pm.Deterministic('gamma_k', pm.math.exp(log_gamma_k))
        pm.Deterministic('alpha_global', pm.math.exp(mu_log_alpha_global))
        pm.Deterministic('beta_global', pm.math.exp(mu_log_beta_global))
        pm.Deterministic('gamma_global', pm.math.exp(mu_log_gamma_global))

        x_pow = x_scaled ** beta_k[keyword_idx]
        mu = alpha_k[keyword_idx] * (x_pow / (gamma_k[keyword_idx] + x_pow))
    ...
    def predict_level_numpy(
        ...
        if level == 'global':
            ...
            gamma = float(posterior_means['gamma_global'])
            x_pow = x_scaled ** beta
            return np.maximum(alpha * (x_pow / (gamma + x_pow)), 0.0)
        if level == 'cluster':
            if indices is None:
            ...
            gamma = np.asarray(posterior_means['gamma_cluster'])[indices]
            x_pow = x_scaled ** beta
            return np.maximum(alpha * (x_pow / (gamma + x_pow)), 0.0)
        if level == 'keyword':
            if indices is None:
            ...
            gamma = np.asarray(posterior_means['gamma_k'])[indices]
            x_pow = x_scaled ** beta
            return np.maximum(alpha * (x_pow / (gamma + x_pow)), 0.0)

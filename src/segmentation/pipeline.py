...
_SEGMENT_COLUMNS = [
    ...
    'M_cum_share', 'is_top_click_group', 'cluster_id', 'cluster_size', 'is_clustered',
]


def _empty_cluster_frame(noise_label: int) -> pd.DataFrame:
    return pd.DataFrame(columns=['keyword', 'cluster_id', 'cluster_size', 'is_clustered']).assign(
...
def _cluster_one_segment(target_df: pd.DataFrame, config: BacktestConfig) -> tuple[pd.DataFrame, dict]:
    ...
    if len(keywords) < max(2, config.semantic.min_cluster_size):
...
def build_segment_table(train_df: pd.DataFrame, config: BacktestConfig) -> pd.DataFrame:
    ...
    if config.segmentation.use_semantic_clustering:
        ...
        for segment_name in semantic_segments:
            target = segment_table.loc[segment_table['segment'].eq(segment_name), ['keyword', 'segment']].copy()
            cluster_df, diag = _cluster_one_segment(target, config)
            ...
            if valid_mask.any():
                cluster_df.loc[valid_mask, 'cluster_id'] = (
                    cluster_df.loc[valid_mask, 'cluster_id'].astype(int) + next_cluster_base
                )
